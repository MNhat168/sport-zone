...
export class Tournament extends BaseEntity {
  ...
  @Prop({ type: Types.ObjectId, ref: 'Transaction' })
  cancellationTransaction?: Types.ObjectId;
...
TournamentSchema.virtual('remainingSpots').get(function () {
  const totalSpots = (this.numberOfTeams || 0) * (this.teamSize || 0);
  const participantsCount = this.participants?.length || 0;
  return Math.max(0, totalSpots - participantsCount);
});
...
TournamentSchema.virtual('isFull').get(function () {
  ...
  const participantsCount = this.participants?.length || 0;
  return participantsCount >= totalSpots;
});

// Virtual for getting formatted team info
TournamentSchema.virtual('teamInfo').get(function () {
  const participantsCount = this.participants?.length || 0;
  const teamSize = this.teamSize || 0;
  const numberOfTeams = this.numberOfTeams || 0;
  const totalSpots = numberOfTeams * teamSize;

  return {
    totalTeams: numberOfTeams,
    teamSize: teamSize,
    currentTeams: teamSize > 0 ? Math.ceil(participantsCount / teamSize) : 0,
    participants: participantsCount,
    remainingSpots: Math.max(0, totalSpots - participantsCount),
    isFull: participantsCount >= totalSpots
  };
});
