...
export class PaymentsController {
  ...
  async handleVNPayCallback(@Query() query: any) {
    ...
    try {
      ...
      // Idempotency: if already processed, acknowledge without changes
      if (payment.status === PaymentStatus.SUCCEEDED || payment.status === PaymentStatus.FAILED) {
        console.log('[VNPay IPN] Payment already processed:', payment.status);
      ...
      if (responseCode === '00') {
        ...
        console.log('[VNPay IPN] ✅ Payment succeeded:', updated._id);

        // Emit payment success event → Triggers booking confirmation
        ...
        console.log('[VNPay IPN] ⚠️ Payment failed:', responseCode);

        // Emit payment failed event → Triggers booking cancellation
      ...
      console.error('[VNPay IPN] Error processing callback:', e);
      // Always acknowledge to VNPay to stop retries
  ...
  async verifyVNPayReturn(@Query() query: any) {
    ...
    console.log('[Verify VNPay] Verifying payment:', {
      orderId,
      responseCode,
      transactionId
    });
